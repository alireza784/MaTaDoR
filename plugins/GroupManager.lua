--Begin GroupManager.lua By @Edited
...
local function delmsg (MaTaDoR,Edited)
    msgs = MaTaDoR.msgs 
    for k,v in pairs(Edited.messages_) do
        msgs = msgs - 1
        tdcli.deleteMessages(v.chat_id_,{[0] = v.id_}, dl_cb, cmd)
        if msgs == 1 then
            tdcli.deleteMessages(Edited.messages_[0].chat_id_,{[0] = Edited.messages_[0].id_}, dl_cb, cmd)
            return false
        end
    end
    tdcli.getChatHistory(Edited.messages_[0].chat_id_, Edited.messages_[0].id_,0 , 100, delmsg, {msgs=msgs})
end
